...
func TestMainWritesTaskfileTextToStdout(t *testing.T) {
	got := captureStdout(t, main)
	if !strings.Contains(got, `- go build -o {{.GOBIN}}/{{.BINARY}} .`) {
		t.Fatalf("main() output missing %q in:\n%s", `- go build -o {{.GOBIN}}/{{.BINARY}} .`, got)
	}
}

func TestRunRendersTaskfileText(t *testing.T) {
	t.Parallel()
